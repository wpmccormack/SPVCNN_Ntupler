  ...
  */
  ...
  for(unsigned int t=0; t<uniqueTrackIDs_TRACEBACK.size();t++){
	...
	if(it.first.first == uniqueTrackIDs_TRACEBACK.at(t) && it.first.second.depth() == d){
	  ...
	  avgX += it.second*normedx;
	  avgY += it.second*normedy;
	  avgZ += it.second*normedz;
	  totalE += it.second;
      ...
      if(atLeast1Hit){
	float normedAvgX = avgX/totalE;
	float normedAvgY = avgY/totalE;
	float normedAvgZ = avgZ/totalE;
	...
	for(unsigned int v = 0; v < energies.size(); v++){
	  etawidth += energies.at(v)*(etas.at(v) - avgEta)*(etas.at(v) - avgEta);
	  float phidiff = abs(phis.at(v) - avgPhi);
	  if(phidiff > M_PI){
	    phidiff = 2.*M_PI - phidiff;
	  }
	  phiwidth += energies.at(v)*phidiff*phidiff;
	}
	etawidth = etawidth/totalE;
	etawidth = sqrt(etawidth);
	phiwidth = phiwidth/totalE;
